...
KernelSU Next hỗ trợ đầy đủ kiến trúc `x86_64`. Tuy nhiên, do các thay đổi bảo mật gần đây từ upstream kernel, việc tích hợp KernelSU Next trên các kernel `x86_64` hiện đại cần thêm xử lý để unified syscall dispatcher của chúng ta hoạt động chính xác.
...
Cơ chế `syscall_hook` của KernelSU Next dựa vào việc sửa đổi các mục trong syscall table để các system call bị chặn có thể được chuyển đến unified dispatcher. Vì cơ chế hardening mới thay đổi đường đi của system call, kernel sẽ bỏ qua các thay đổi đó đối với syscall table. Nếu KernelSU Next cố tải và hook syscall table mà không xử lý giới hạn này đúng cách, nó sẽ không thể định tuyến lời gọi và sẽ chủ động hủy khởi tạo để tránh kernel panic.
...
Một ghi chú từ thượng nguồn (upstream):
...
Để KernelSU Next hoạt động trên các kernel mới này, hãy áp dụng bản vá cho phép bỏ qua cơ chế syscall hardening cụ thể này.
